# 1. 題目說明:
# 請開啟PYD706.py檔案，依下列題意進行作答，進行全字母句之判斷，
# 使輸出值符合題意要求。作答完成請另存新檔為PYA706.py再進行評分。

# 2. 設計說明：
# 全字母句（Pangram）是英文字母表所有的字母都出現至少一次
# （最好只出現一次）的句子。請撰寫一程式，要求使用者輸入一正整數k
# （代表有k筆測試資料），每一筆測試資料為一句子，
# 程式判斷該句子是否為Pangram，並印出對應結果True（若是）
# 或False（若不是）。
