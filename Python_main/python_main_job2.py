...
with open(file2,encoding="utf-8") as f:
    ...
    df1_city = df[["臺北市_Taipei_City","臺中市_Taichung_City",
                  "臺南市_Tainan_City","高雄市_Kaohsiung_City","彰化縣_Changhua_County"]]
    df1_county = df[["新北市_New_Taipei_City","臺中縣_Taichung_County",
                    "臺南縣_Tainan_County","高雄縣_Kaohsiung_County","新竹市_Hsinchu_City"]]
    ...
    # 目前中華民國共有六個直轄市：臺北市（1967年改制）、高雄市（1979年改制，2010年與高雄縣合併）、
    # 新北市（2010年改制）、臺中市（2010年改制）、臺南市（2010年改制）、桃園市（2014年改制），俗稱「六都」。
    ...
    df1_Taiwan = df1_Taiwan[df1_Taiwan.index >= "2015/1"] # 這裡設定跑圖的起使
    df1_Taiwan = df1_Taiwan[df1_Taiwan.index <= "2015/1"] # 這裡設定跑圖的結束
    
    df1_county = df1_county[df1_county.index >= "2015/1"] # 這裡設定跑圖的起使
    df1_county = df1_county[df1_county.index <= "2015/1"] # 這裡設定跑圖的結束
    ...
    df1_county_ans = round(pd.DataFrame(df1_county_ans,columns=["失業率"]),2) # 轉pd二維及限制小數位

    print(df1_Taiwan.sum().head(),"取得加總值") #取五筆檢視就好
    print(df1_Taiwan.count().head().values,"取得加總")
    # print(df1_county_ans) #檢視格式OK
    print(df1_county_ans.sum().head(),"取得加總值") #取五筆檢視就好
    print(df1_county_ans.count().head().values,"取得加總")
    ...
    # 視覺化:長條圖(比較圖)
    width = 0.3 #條寬度
    # 臺北市_Taipei_City","臺中市_Taichung_City",
                  # "臺南市_Tainan_City","高雄市_Kaohsiung_City","彰化縣_Changhua_County"]]
    ax_name = ['台北/新北市', '台中市/縣', '臺南市/縣', '高雄市/縣', "彰化/新竹"] # 底下X軸名稱自訂
    x = np.arange(len(ax_name))
    fig, ax = plt.subplots()
    rects1 = ax.bar(x - width/2, df1_col_values, width, label='市')
    rects2 = ax.bar(x + width/2, df1_col_values2, width, label='縣')
    
    ax.set_ylabel("失\n業\n率",size=14,rotation=0,ha="right",color="r")
    ax.set_title('失業率 - 縣市別')
    ax.set_xticks(x)
