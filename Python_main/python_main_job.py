...
with open(file2,encoding="utf-8") as f:
    ...
    #     str_list.append(i[:4])
    # print(str_list)
    df["年份"] = df["年份"].str.replace("Jan.-June","/1月-6月").str.replace("July-Dec.","/7月-12月") #處理結構，設定索引
    ...
    # 計算縣市的失業率
    print(df1_Taiwan.sum().head(),"取得加總")
    print(df1_Taiwan.count().head().values,"取得數量")
    df1_Taiwan_ans = df1_Taiwan.sum() / df1_Taiwan.count().values # 加總的值 / 加總的數量(並取得值)
    df1_Taiwan_ans = pd.DataFrame(df1_Taiwan_ans,columns=["年份"])
    ...
    # # plt.ylabel("😂",fontname="symbola",size=16,rotation=0,ha="right") #Y軸
    # plt.ylabel("失\n業\n率",size=16,rotation=0,ha="right",color="r") #Y軸
    # plt.title(df1_Taiwan_name03,y=1.01,size=18) #標題
    # plt.legend(["男生","女生"],loc="upper left",fontsize=14) #小圖位置
    # plt.xlabel("日期",size=16) #X軸標題
    # plt.grid(True,color="black",linewidth=0.6) #網格
    # plt.gca().set_facecolor("black") #背景顏色
